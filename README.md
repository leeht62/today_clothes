...
날씨 정보를 기반으로 AI 코디 이미지를 추천하고, 추천 결과를 게시판에 공유하며, 댓글과 좋아요 및 실시간 알림까지 제공하는 서비스입니다.  
Spring Boot, FastAPI, PostgreSQL, Redis, WebSocket을 함께 사용해 백엔드와 AI 서버를 분리한 구조로 구현했습니다.

---

## 1. 프로젝트 소개

`Today Clothes`는 사용자의 날씨 상황에 맞는 옷차림을 추천해주는 서비스입니다.

사용자는 로그인 후 날씨 기반 AI 코디 이미지를 추천받을 수 있고, 생성된 결과를 저장하거나 게시판에 공유할 수 있습니다.  
다른 사용자는 게시글에 댓글을 남기고 좋아요를 누를 수 있으며, 이 과정에서 Redis와 WebSocket을 활용한 실시간 알림 기능도 함께 제공합니다.
...
![WebSocket](https://img.shields.io/badge/websocket-%23ED8B00.svg?style=for-the-badge&logo=websocket&logoColor=white)
...
<div align="center">
  <img width="500" height="855" alt="today-clothes architecture" src="https://github.com/user-attachments/assets/19fb0303-0794-46cc-aa03-df5c9487a9ed" />
</div>

### 서비스 구성

- `Frontend (React)`  
  사용자 UI를 제공하며 Spring Boot 서버와 통신합니다.

- `Spring Boot Server`  
  회원 관리, 인증, 게시판, 댓글, 좋아요, 알림, AI 서버 연동을 담당합니다.

- `FastAPI Server`  
  날씨 기반 코디 추천 이미지와 관련 응답을 생성합니다.

- `PostgreSQL`  
  사용자, 게시글, 댓글, 추천 결과 등의 영속 데이터를 저장합니다.

- `Redis`  
  좋아요 집계, 캐시, Pub/Sub 기반 실시간 알림 처리에 사용됩니다.
...
| AI 코디 생성 | 생성된 코디 확인 | 게시글 작성 |
| :---: | :---: | :---: |
| ![ai 코디 생성](https://github.com/user-attachments/assets/b430ae76-f43c-4d1f-be42-84bbede28a4e) | ![생성된 코디 확인](https://github.com/user-attachments/assets/23d69d5d-dd14-4c15-bf91-5903593a8cd2) | ![게시글 작성](https://github.com/user-attachments/assets/d227a56b-cfce-4091-bab2-b03d4aee286e) |
